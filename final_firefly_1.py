...
FIREBASE_API_KEY = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..cHVbdVAgeUhSDu3E-7eD3bWwnhXtf9bJeb-PhU_z-S-U1z7SGAUOi4QV3t7qzTtiBZ5jIUUKj97U3euY5Y9I4n9kKlPUUK2_ot49uUDe6NjjvXhDtI-NjEj0jH_hnuQ6A-kaJ3UO_xPftOOm0TCK9ftWj4yrbTKiqNRgPJdOGKzFzoZdUQEAbVmOku6M1ab-LwV5j2BZ8CXBMAPQRnZQqLHmL7pMWpRnydbW_nLEUlcs-yA0d5YBc3aVVm7J5WD9Rfn-a6X_QPxWcQKCDJlH2PNXuxKhc7m3VqEFzTppIeEOO6M5XUB-cpdflJ-bNR2gPWLZJbuJj_tMfGiWfpXonfyzz40vIYE6EO3Qf1wfM76836XxENt_CrknXmXz-jHM7IEA-h5l94RZsRCpwvEdCm-Ui99yLu8tDc4FUAPuU6YUSsCf2RnbYZ-Y4_2741Ou-Ofu1mfw5Ama1E32U-P0V2SKSEzt9SBoPMDs80vgKzmWrkzK3ggnECNEmvcK7p0G4y3nTsGr_YNQxL92oyRzwoC42_oFti7WalY3IL3NQGayMLNhvmrO5e2cgO358I4ZgkWhBS7dOLw-H-POGxiw09NVtBXg9nHA-z3xy_X1yhvzz8rno3ui3-28PH9WbiW1o4HC1IPfTZQz74Udq6j9-1KQeZBsxXf0nRCen3rZfo8"
...
if st.button("지출 조회"):
    ...
    if records:
        ...
        category_mapping = {
            "식사": "식비", "외식": "식비", "식비": "식비",
            "쇼핑": "쇼핑", "의복": "쇼핑",
            "생활구매": "생활비", "생활": "생활비", "미용": "생활비", "생활비": "생활비",
            "차량": "교통비", "교통비": "교통비",
            "업무": "기타", "여행": "취미", "여가": "취미",
            "문화": "취미", "취미": "취미", "의료": "의료", "교육": "교육", "기타": "기타"
        }
        st.write("사용자 지출 내 카테고리 목록:", df["카테고리"].unique())
        df["대분류"] = df["카테고리"].map(category_mapping).fillna("기타")

        file_mapping = {
            "이천시": "synthetic_consumption_final_converted.xlsx",
            "서울시": "synthetic_consumption_final_converted.xlsx"
        }

        # 사용자 선택에 따른 시트 이름
        selected_file = file_mapping.get(data_source, "synthetic_consumption_final_converted.xlsx")
        ...
        try:
            ...
            my_rank = group_sum_sorted[group_sum_sorted["총공공지출"] >= my_total].index.min() + 1
            percentile = (my_rank / len(group_sum_sorted)) * 100 if my_rank else 100

            st.write(f"📊 전체 평균 지출: {overall_avg:,.0f} 원")
            st.write(f"📍 평균보다 {abs(percent_diff):.1f}% {'더 사용' if percent_diff > 0 else '덜 사용'}하셨습니다.")
            st.write(f"📍 전체 유저 중 상위 {percentile:.1f}%에 해당합니다.")
